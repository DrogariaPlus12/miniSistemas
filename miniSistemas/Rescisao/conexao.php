...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $Pendente = (isset($_POST["Pendente"]) && $_POST["Pendente"] != null) ? $_POST["Pendente"] : NULL;
} else if (!isset($id)) {
...
try {
    ...
    $statement = $connection->prepare("SELECT * FROM dbo.DadosRescisao;");
...
if (isset($_REQUEST["act"]) && $_REQUEST["act"] == "save" && $NomeTitular != "") {
    try {
        if ($id != "") {
            $stmt = $connection->prepare("UPDATE dbo.DadosRescisao SET CPF=?, NomeTitular=?, Observacao=?, Pendente=? WHERE id = ?");
            $stmt->bindParam(5, $id);
        } else {
            $stmt = $connection->prepare("INSERT INTO dbo.DadosRescisao (CPF, NomeTitular, Observacao, Pendente) VALUES (?, ?, ?, ?)");
        }
        $stmt->bindParam(1, $CPF);
        $stmt->bindParam(2, $NomeTitular);
        $stmt->bindParam(3, $Observacao);
        $stmt->bindParam(4, $Pendente);
...
if (isset($_REQUEST["act"]) && $_REQUEST["act"] == "upd" && $id != "") {
    try {
        $stmt = $connection->prepare("SELECT * FROM dbo.DadosRescisao WHERE id = ?");
...
if (isset($_REQUEST["act"]) && $_REQUEST["act"] == "del" && $id != "") {
    try {
        $stmt = $connection->prepare("DELETE FROM dbo.DadosRescisao WHERE id = ?");
        $stmt->bindParam(1, $id, PDO::PARAM_INT);
        if ($stmt->execute()) {
