...
<form action="?act=save" method="POST" name="form1" >
               ...
               ?> />
                Codigo do Socio:
               <input type="text" name="CodigoSocio" <?php
 
             
               if (isset($CodigoSocio) && $CodigoSocio != null || $CodigoSocio != "") {
                   echo "value=\"{$CodigoSocio}\"";
               }
               ?> />
                Nome do Socio:
               <input type="text" name="NomeSocio" <?php
 
               if (isset($NomeSocio) && $NomeSocio != null || $NomeSocio != "") {
                   echo "value=\"{$NomeSocio}\"";
               }
               ?> />
            ...
            <table border="1" width="100%">
                ...
                try {
                    $stmt = $connection->prepare("SELECT * FROM dbo.DadosSocios");
                    if ($stmt->execute()) {
                        while ($rs = $stmt->fetch(PDO::FETCH_OBJ)) {
                            echo "<tr>";
                            echo "<td>".$rs->CPF."</td><td>".$rs->CodigoSocio."</td><td>".$rs->NomeSocio
